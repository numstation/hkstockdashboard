...
for f in daily_scan.json daily_scan_sell_put.json daily_scan_buy_stock.json daily_scan_buy_put.json \
  macro_snapshot.json market_catalysts_hk.json signals_history.json breadth_daily_history.json \
  score_daily_history.json future_log.json closed_transactions.json; do
  if [[ -f "$ROOT/$f" ]]; then
